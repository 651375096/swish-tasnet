...
database_path ="/home/chenyang/Dataset/非屏蔽双绞线电磁信号/"
tt_mix_path="/home/chenyang/wave-tasnet/voice_data/wav_sjx_all/tt/mix/"
tr_mix_path="/home/chenyang/wave-tasnet/voice_data/wav_sjx/tr/mix/"
cv_mix_path="/home/chenyang/wave-tasnet/voice_data/wav_sjx/cv/mix/"

tt_s1_path="/home/chenyang/wave-tasnet/voice_data/wav_sjx_all/tt/s1/"
tr_s1_path="/home/chenyang/wave-tasnet/voice_data/wav_sjx/tr/s1/"
cv_s1_path="/home/chenyang/wave-tasnet/voice_data/wav_sjx/cv/s1/"

tt_s2_path="/home/chenyang/wave-tasnet/voice_data/wav_sjx_all/tt/s2/"
tr_s2_path="/home/chenyang/wave-tasnet/voice_data/wav_sjx/tr/s2/"
cv_s2_path="/home/chenyang/wave-tasnet/voice_data/wav_sjx/cv/s2/"

for dstpath in [tt_mix_path,tr_mix_path,cv_mix_path,tt_s1_path,tr_s1_path,cv_s1_path,tt_s2_path,tr_s2_path,cv_s2_path]:
    if not os.path.exists(dstpath):
        os.makedirs(dstpath)  # 创建路径
...
for _, folders, files in os.walk(database_path):
    for file_name in sorted(files):
        if file_name.endswith('.csv'):
            ...
            aaa,bbb,ccc,aaa_cd = read3CSV(subset111_folder)

            np.savetxt(tt_mix_path + "/" + file_name  + ".txt",
                       ccc, fmt='%.05f')
            np.savetxt(tt_s1_path + "/" + file_name  + ".txt",
                       aaa, fmt='%.05f')
            np.savetxt(tt_s2_path + "/" + file_name + ".txt",
                       bbb, fmt='%.05f')
            #
            # slice_cd=500000
            ...
            #         s1_path = tr_s1_path
            #         s2_path = tr_s2_path
            #     huatu(ccc[aaa_cd * ai // slic_cs:aaa_cd * (ai + 1) // slic_cs],"sjx-aaa")
            #     huatu(aaa[aaa_cd * ai // slic_cs:aaa_cd * (ai + 1) // slic_cs], "sjx-s1-ori")
            #     huatu(bbb[aaa_cd * ai // slic_cs:aaa_cd * (ai + 1) // slic_cs], "sjx-s2-ori")
            #     np.savetxt(mix_path + "/" + file_name + str(ai) + ".txt",
            #                ccc[aaa_cd * ai // slic_cs:aaa_cd * (ai + 1) // slic_cs], fmt='%.05f')
            #     np.savetxt(s1_path + "/" + file_name + str(ai) + ".txt",
            #                aaa[aaa_cd * ai // slic_cs:aaa_cd * (ai + 1) // slic_cs], fmt='%.05f')
            #     np.savetxt(s2_path + "/" + file_name + str(ai) + ".txt",
            #                bbb[aaa_cd * ai // slic_cs:aaa_cd * (ai + 1) // slic_cs], fmt='%.05f')
# zmb2=readLvm("/home/chenyang/wave-tasnet/voice_data/jmdx (1).lvm")
#
# huatu(zmb2['frame'],"zmb2['frame']")
# huatu(zmb2['signal'],"zmb2['signal']")
