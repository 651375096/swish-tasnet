...
def handleBinarization(data,input_standard,deviation,way=0):
    ...
    if isNumber(input_standard):
        ...
        standard = np.max(bin_data)-(np.max(bin_data)/3)  # 最小值作为标准
...
###########
        ...
        for i11 in range(len(hang_poisition)):
            for i22 in range(hang_poisition[len(hang_poisition)-1]["len"]):
                ...
                fuxiasn11[i11][i22]=fuxiasn[hang_poisition[i11]["start"]+i22]
...
#--------------------------------------------------------------------------------------------------------------------------------
#获取数据集
Fuxian_path="/home/chenyang/wave00/out/中间汉字基本笔画书写1024-5层-elu-signal-noise-13 best-2.2845983055969693e-08/中间汉字基本笔画书写_0.05/signal.txt"
wave_wjm = "Linuxtest"
...
blue_frame=readLvm(blue_path)
line_frame = readLvm(hang_path)
fuxiasn=np.loadtxt(Fuxian_path)
getDataset1()
