...
def handleBinarization(data,input_standard,deviation,way=0):
    ...
    if isNumber(input_standard):
        ...
        # standard=-0.0022#linux机子用
        # standard = (np.max(bin_data) + np.min(bin_data)) / 2
        # standard =  -np.average(bin_data) / 2
        # standard = ( np.min(bin_data)*0.13)
        # print("np.max(bin_data)) ", np.max(bin_data))#0.00535
        print("np.minbin_data)) ", np.min(bin_data))#-0.01278
        # print("np.average(bin_data)) ",np.average(bin_data))## 1.7529470019691476e-05
...
def getDataset1():
    ...
    for zhen_cs in range(part_num):
        ...
        subDir_wav="./voice_data/"+wave_wjm +"wav/"
...
###########
        ...
        fuxiasn11=np.zeros((len(hang_poisition),hang_poisition[len(hang_poisition)-1]["maxlen"]), dtype=np.float64)
        fuxiasn22=np.zeros((len(hang_poisition),hang_poisition[len(hang_poisition)-1]["maxlen"]), dtype=np.float64)
        ...
        for i11 in range(len(hang_poisition)):
            for i22 in range(hang_poisition[len(hang_poisition)-1]["len"]):
                ...
                fuxiasn11[i11][i22]=fuxiasn1[hang_poisition[i11]["start"]+i22]
                fuxiasn22[i11][i22]=fuxiasn2[hang_poisition[i11]["start"]+i22]

        # fuxian(fuxiasn11, "fuxiasn11原版")
        huatu(fuxiasn11,"fuxiasn11之前")
        # fuxiasn11[np.where(fuxiasn11 <np.max(fuxiasn11)/10)] = 0
        # # fuxiasn11[np.where(fuxiasn11 <np.average(fuxiasn11))] = np.min(fuxiasn11)
        # # fuxiasn11[np.where(fuxiasn11 <np.average(fuxiasn11))] = 0
        # fuxiasn11[np.where(fuxiasn11 != 0)] = 1
        # huatu(fuxiasn11, "fuxiasn11之后")
        # zscc*=1000


        print(zscc, zscc.shape)
        # zscc = 255 * zscc/ (np.max(zscc) - np.min(zscc))
        fuxian(zscc, "fuxian_jmu")
        # fuxian1(zscc, "fuxian_jmu1")

        fuxian(zscc11, "fuxian_blue_pure")
        # fuxian1(zscc11, "fuxian_blue_pure1")


        fuxian(fuxiasn11, "fuxiasn11")
        fuxian1(fuxiasn11, "fuxiasn111")
        fuxian(fuxiasn22, "fuxiasn22")
        fuxian1(fuxiasn22, "fuxiasn222")
        pdb.set_trace()
...
#--------------------------------------------------------------------------------------------------------------------------------
#获取数据集
Fuxian1_path="/home/chenyang/wave-tasnet/exp/tmp/out/signal/estimate/s1/中间汉字基本笔画书写1帧.txt"
Fuxian2_path="/home/chenyang/wave-tasnet/exp/tmp/out/signal/estimate/s2/中间汉字基本笔画书写1帧.txt"
# Fuxian_path="/home/chenyang/wave00/out/中间汉字基本笔画书写1024-5层-elu-signal-noise-13 best-2.2845983055969693e-08/中间汉字基本笔画书写_0.05/0noise.txt"
zhen_cs_zhid=1
wave_wjm = "Linuxtest"
# mix_path="../wave00/16data/1024_768_60_jmu_250MHz.lvm"
# blue_path="../wave00/16data/1024_768_60_B_250MHz.lvm"
# hang_path="../wave00/16data/1024_768_60_ZHENHANG_250MHz.lvm"
# mix_path="../各种数据集数据库/VGA/Linux主机-HP LE1901W/中间集美大学/1024_768_60Hz_通道0为帧同步_通道1为电磁信号.lvm"
# blue_path="../各种数据集数据库/VGA/Linux主机-HP LE1901W/中间集美大学/1024_768_60Hz_通道0为帧同步_通道1为蓝信号.lvm"
# hang_path="../各种数据集数据库/VGA/Linux主机-HP LE1901W/中间集美大学/1024_768_60Hz_通道0为帧同步_通道1为行信号.lvm"
mix_path="../各种数据集数据库/VGA/Linux主机-HP LE1901W/中间汉字基本笔画书写/1024_768_60.00Hz_通道0为帧同步_通道1为电磁信号.lvm"
blue_path="../各种数据集数据库/VGA/Linux主机-HP LE1901W/中间汉字基本笔画书写/1024_768_60.00Hz_通道0为帧同步_通道1为蓝信号.lvm"
hang_path="../各种数据集数据库/VGA/Linux主机-HP LE1901W/中间汉字基本笔画书写/1024_768_60.00Hz_通道0为帧同步_通道1为行信号.lvm"
# mix_path="../各种数据集数据库/VGA/联想a机+HP LE1901W/联想a机外网/1024 250mhz 0帧同步1混合信号.lvm"
# blue_path="../各种数据集数据库/VGA/联想a机+HP LE1901W/联想a机外网/1024 250mhz 0帧同步1蓝信号.lvm"
# hang_path="../各种数据集数据库/VGA/联想a机+HP LE1901W/联想a机外网/1024 250mhz 0帧同步1行同步.lvm"
jmu_frame=readLvm(mix_path)
blue_frame=readLvm(blue_path)
line_frame = readLvm(hang_path)
fuxiasn1=np.loadtxt(Fuxian1_path)
fuxiasn2=np.loadtxt(Fuxian2_path)
