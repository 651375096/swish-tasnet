...
def fuxian(data,name):
    ...
    data[np.where(data<np.average(data)/3)]=0
...
def handleBinarization(data,input_standard,deviation,way=0):
    ...
    if isNumber(input_standard):
        ...
        print("input_standard==max",standard)
...
def getDataset_full_30():
    ...
    # 利用叠加的掩码文件从jmu信号中获取所需的理想纯净信号
    mask_hang = getMarsk(line, bin_standard, bin_deviation, replace_point, point_num, aim_value)
    # huatu(mask_hang[0:50000],"mask_hang[0:50000]")

    mask_zhen = getMarsk(zhen_frame, bin_standard, bin_deviation, replace_point, point_num, aim_value)
    # huatu(mask_zhen,"mask_zhen")

    reverse_mask_blue = reverseMatrix(mask_blue)
    blue_noise_blank = reverse_mask_blue * jmu
    # huatu(blue_noise_blank[2999000:3000000],"blue_noise_blank[2990000:3000000] ")
    distance = 200
    blue_noise = fillBlanks(mask_blue, blue_noise_blank, distance)
    # huatu(blue_noise,"blue_noise")
    # 数据集的制作
    ...
    s2 = blue_noise
    mix = jmu
    huatu(jmu[200000:220000],"jmu[2000000,2009000]")

    huatu((mask_hang + mask_zhen)[200000:220000], "mask_hang+mask_zhen[2000000,2009000]")
    huatu(hang[200000:220000], "hang[2000000,2009000]")


    huatu(reverse_mask_blue[2000000: 2009000], "reverse_mask_blue[2000000,2009000]")
    huatu(blue_noise[2000000: 2009000], "blue_noise[2000000,2009000]")


    huatu(mask_blue[2000000: 2009000], "mask_blue[2000000,2009000]")
    huatu(blue_pure[2000000: 2009000], "blue_pure[2000000,2009000]")
    ...
    for zhen_cs in range(zon_cs):
        if ( zhen_cs < int(zon_cs * 0.2)):
            subDir2 = "cv/"
        elif ( zhen_cs  < int(zon_cs * 0.2)+1):
            subDir2 = "tt/"
        elif ( zhen_cs  < int(zon_cs * 0.4)):
            subDir2 = ""
        else:
            subDir2 = "tr/"
        subDir3_1 = "s1/"
...
def pre_out():
    ...
    # huatu(line[1000000:1050000],"line[1000000:1050000]")
    bin_standard = hang_bin_standard
    print("bin_standard", bin_standard)
    # pre_hang[np.where(pre_hang_> 0.01)] = np.average(pre_hang)
    # pre_hang[np.where(pre_hang< -0.01)] = np.average(pre_hang)
    print(np.average(pre_hang),"np.average(pre_hang)")

    # huatu(pre_hang[1000000:1050000],"[1000000:1050000]!= 0)] = 1")
    hang_poisition = gethangInfornamtion_zf(pre_hang_original, bin_standard)
    ...
    for i11 in range(len(hang_poisition)):
        for i22 in range(hang_poisition[len(hang_poisition) - 1]["len"]):
            ...
            # zscc11[i11*kuo_h][i22*kuo_l] = pre_noise[hang_poisition[i11]["start"] + i22]
    fuxian(zscc, fbl + "pre_mix")
    fuxian(zscc11, fbl + "pre_blue_original")
    fuxian(zscc_blue, fbl + "fuxian_blue")
    # fuxian(zscc_noise, fbl + "fuxian_noise")
...
fbl_extra ="3ppt"
fbl="aa"
# database_path="/home/dcxx/All_Dataset/308VGA4_in_side"
# database_path ="/home/chenyang/Dataset/VGA/308VGA4_in_side"
database_path ="/root/autodl-tmp/Dataset_All/308VGA4_inside"
# database_path ="/home/dcxx/chenyang/Dataset/VGA/308VGA4_in_side"
out_path = "./exp/tmp/out/signal/estimate/s1"
...
# #
# #
# zmb2=readLvm("/home/chenyang/Dataset/VGA/ppt2/1024_768/20220401_1024-768zhen+fushe+ppt2.lvm")
# np.savetxt("./zmb.txt",zmb2['frame'][0:1500000])
# huatu(zmb2['frame'][0:15000000],"zmb2")
# huatu(zmb2['signal'][0:15000000],"zmb3")

# # ###############

# pre_name="1.1-48-4D-7E-BE-C8-651024-768.csv14"
# hang_bin_standard="pre_out"###决定二值化标准
# pre_mix_path="./voice_data/wav/tt/mix/"+pre_name +  ".txt"
# pre_mix=np.loadtxt(pre_mix_path)
# # huatu(pre_mix[200000:220000],"premix")
# # huatu(pre_mix,"pre_mix")
#
# pre_blue_s1_path="./exp/tmp/out/signal/estimate/s1/"+pre_name+  ".txt.txt"
# pre_blue=np.loadtxt(pre_blue_s1_path)
# # huatu(pre_blue,"pre_blue")
#
# pre_blue_original_s1_path="./exp/tmp/out/signal/original/s1/"+pre_name+  ".txt.txt"
# pre_blue_original=np.loadtxt(pre_blue_original_s1_path)
# # huatu(pre_blue_original,"pre_blueoriginal")
#
#
# pre_hang_s3_path= "./exp/tmp/out/signal/estimate/s3/"+pre_name+  ".txt.txt"
# pre_hang=np.loadtxt(pre_hang_s3_path)
# huatu(pre_hang,"pre_hang")
# huatu(pre_hang[200000:220000],"pre_hang")
#
#
# pre_hang_original_s3_path="./exp/tmp/out/signal/original/s3/"+pre_name+".txt.txt"
# pre_hang_original=np.loadtxt(pre_hang_original_s3_path)
# huatu(pre_hang_original[200000:220000],"pre_hang_ooriginal")
# pre_out()
# #
#
####################