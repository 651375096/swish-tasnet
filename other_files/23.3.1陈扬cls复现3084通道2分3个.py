...
###########
        ...
        for i11 in range(len(hang_poisition)):
            for i22 in range(hang_poisition[len(hang_poisition)-1]["len"]):
                ...
                zscc11[i11][i22]=c12[hang_poisition[i11]["start"]+i22]
        ...
        if subDir2 == "tt/":
            ...
            sliceData_CYtasnet(filePath_wav_mix_sum, mix, "wav", file_name=fbl + str(zhen_cs) + "帧", file_end=".stem",
                               slice_size=slice_size)
            sliceData_CYtasnet(filePath_wav_signal_sum, c12, "wav", file_name=fbl + str(zhen_cs) + "帧",
                               file_end=".stem_signal", slice_size=slice_size)
            sliceData_CYtasnet(filePath_wav_noise_sum, blue_noise, "wav", file_name=fbl + str(zhen_cs) + "帧",
                               file_end=".stem_noise", slice_size=slice_size)
            sliceData_CYtasnet(filePath_wav_hang_sum, hang, "wav", file_name=fbl + str(zhen_cs) + "帧",
                               file_end=".stem_noise", slice_size=slice_size)
    print("结束")
#--------------------------------------------------------------------------------------------------------------------------------
#获取数据集
hang_bin_standard="308VGA4通道内存"###决定二值化标准
fbl ="test"

all_path="308VGAxiao内存数据"
database_path="/home/chenyang/各种数据集数据库/VGA/308VGA4通道内存数据"
    ...
    for track_name in sorted(folders):
        ...
        for _, folders, files in os.walk(subset_folder):
            for file_name in sorted(files):
                if file_name.endswith('1024-768.csv'):
                    print(1)
...
###########################
# allpath="/home/chenyang/各种数据集数据库/RigolDS3.csv"
