...
def fuxian(data,name):
    ...
    # data = 1 * data / (np.max(data) - np.min(data))
...
def getDataset_full_30():
    ...
    aim_value = 1  # 替换值
    # 获取相应的掩码
    mask_jmu = getMarsk(jmu, bin_standard, bin_deviation, replace_point, point_num, aim_value)
    ...
    huatu(mask_jmu,"mask_jmu")
    huatu(mask_blue,"mask_blue")
    huatu(mask_hang,"mask_hang")
    ...
    fuxian(zscc[0:hang_poisition], fbl + "fuxian_jmu")
    fuxian(zscc_blue[0:hang_poisition], fbl + "fuxian_blue")
    fuxian(zscc11[0:hang_poisition], fbl + "fuxian_c12")
    ...
    for zhen_cs in range(zon_cs):
        ...
        if (subDir2 == "tt/"):
            ...
            writeData(jmu[0:(zhen_cs+1)*slice_size], file_form, filePath_wav_mix_sum, fbl + str(zhen_cs) + "")
            writeData(c12[0:(zhen_cs+1)*slice_size], file_form, filePath_wav_signal_sum, fbl + str(zhen_cs) + "")
            writeData(blue_noise[0:(zhen_cs+1)*slice_size], file_form, filePath_wav_noise_sum, fbl + str(zhen_cs) + "")
            writeData(hang[0:(zhen_cs+1)*slice_size], file_form, filePath_wav_hang_sum, fbl + str(zhen_cs) + "")
        elif (subDir2 == "cv/") :
            print(zon_cs, "中的第", zhen_cs, subDir2)
...
def pre_out():
    ...
    bin_standard = hang_bin_standard
    print("bin_standard", bin_standard)
    pre_hang[np.where(pre_hang> np.max(pre_hang)/10)] = 1
    pre_hang[np.where(pre_hang< np.min(pre_hang)/10)] = 1

    huatu(pre_hang[1000000:1050000],"[1000000:1050000]!= 0)] = 1")
    hang_poisition = gethangInfornamtion_zf(pre_hang, bin_standard)
    print(hang_poisition, "hang_poisition maxlen=", hang_poisition[5]["maxlen"], "len=", len(hang_poisition))
    kuo_h = 1
    kuo_l = 1
    x,y1,y2,y3=[0],[0],[0],[0]
    ...
    for i11 in range(len(hang_poisition)):
        ...
        for i22 in range(hang_poisition[len(hang_poisition) - 1]["len"]):
            # x.append(i11)
            # y1.append(pre_mix[hang_poisition[i11]["start"] + i22])
            zscc[i11*kuo_h][i22*kuo_l] = pre_mix[hang_poisition[i11]["start"] + i22]
            # y2.append(pre_blue[hang_poisition[i11]["start"] + i22])
            zscc_blue[i11 *kuo_h][i22*kuo_l] = pre_blue[hang_poisition[i11]["start"] + i22]
            # y3.append( pre_blue_original[hang_poisition[i11]["start"] + i22])
...
fbl_extra ="3ppt"
fbl="aa"
# database_path_test="./voice_data/wav/tt"
# database_path="/home/dcxx/All_Dataset/308VGA4_in_side"
# database_path="/root/autodl-tmp/Dataset/chenyang_test"
out_path = "/root/autodl-tmp/chenyang/tasnet/exp/tmp/out/signal/estimate/s1"
...
# pre_mix_path="/home/chenyang/wave-tasnet/voice_data/wav/tt/mix/1.1-48-4D-7E-BE-C8-651024-768.csv12.txt"
# pre_mix=np.loadtxt(pre_mix_path)
# huatu(pre_mix,"pre_mix")
#
# pre_blue_s1_path="/home/chenyang/wave-tasnet/exp/tmp/out/signal/estimate/s1/1.1-48-4D-7E-BE-C8-651024-768.csv12.txt.txt"
# pre_blue=np.loadtxt(pre_blue_s1_path)
# huatu(pre_blue,"pre_blue")
#
# pre_blue_original_s1_path="/home/chenyang/wave-tasnet/exp/tmp/out/signal/original/s1/1.1-48-4D-7E-BE-C8-651024-768.csv12.txt.txt"
# pre_blue_original=np.loadtxt(pre_blue_original_s1_path)
# huatu(pre_blue_original,"pre_blueoriginal")
#
# pre_hang_s3_path="/home/chenyang/wave-tasnet/exp/tmp/out/signal/estimate/s3/1.1-48-4D-7E-BE-C8-651024-768.csv12.txt.txt"
# pre_hang=np.loadtxt(pre_hang_s3_path)
# huatu(pre_hang,"pre_hang")
# huatu(pre_hang[1000000:1050000],"pre_hang[1000000:1050000]")
#
# pre_hang_original_s3_path="/home/chenyang/wave-tasnet/exp/tmp/out/signal/original/s3/1.1-48-4D-7E-BE-C8-651024-768.csv12.txt.txt"
# pre_hang_original=np.loadtxt(pre_hang_original_s3_path)
# huatu(pre_hang_original,"pre_hang_ooriginal")
