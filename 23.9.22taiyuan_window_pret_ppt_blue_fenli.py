...
def read4CSV_taiyuan_vga2_mix(path):
    aaa,bbb,ccc,ddd=[]*1,[]*1,[]*1,[]*1
    # with open(path, 'r', encoding='GBK') as fp:
    with open(path, 'r', encoding='utf-8') as fp:
        reader = csv.reader(fp)
...
def read4CSV_taiyuan_vga2_usb(path):
    aaa,bbb,ccc,ddd=[]*1,[]*1,[]*1,[]*1
    # with open(path, 'r', encoding='GBK') as fp:
    with open(path, 'r', encoding='utf-8') as fp:
        reader = csv.reader(fp)
...
def read4CSV_5_11_18_24(path):
    ...
    with open(path, 'r', encoding='utf-8') as fp:
        ...
        for x in reader:
            i += 1
            if i > 3:
                aaa.append(float(x[4]))
                bbb.append(float(x[10]))
                ccc.append(float(x[16]))
                ddd.append(float(x[22]))
    fram1 = np.array(aaa)
    fram2 = np.array(bbb)
    fram3 = np.array(ccc)
    fram4 = np.array(ddd)
...
def fuxian_taiyuan(data,name,beishu=1):

    # data = 255/ (np.max(data) - np.min(data)) * (data)
    data[np.where(data<np.average(data))]=0
    data = 255 / (np.max(data) - np.min(data)) * (data)
    # data[np.where(data>0)]=255
    # data[np.where(data<np.max(data)-np.average(data))]=0
    data*=beishu
...
def handleBinarization(data,input_standard,deviation=0,way=0):
    ...
    if isNumber(input_standard):
        ...
        standard =-0.001
    elif input_standard == "pre_out_taiyuan":
        # standard =np.max(bin_data)/5
        # standard=1
        # standard = 0.01
        # standard = -0.05
    ...
    elif input_standard == "pre_out_blue_taiyuan":
        # standard =np.max(bin_data)/5
        # standard =np.max(bin_data)/4####摄像机VGA二值化

        standard =np.max(bin_data)/2####摄像机VGA二值化
        # standard=1
        # standard = 0.01
        # standard = -0.01
...
def getDataset_full_30():
    ...
    bin_standard = "pre_out_blue_taiyuan"  # 进行二值化的标准
    ...
    slice_size = hang_poisition[len(hang_poisition) - 1]["maxlen"] * 75
    zon_cs=int(zon_cd//slice_size)
    print("zon_cs",zon_cs)

    for zhen_cs in range(zon_cs):
        if (zhen_cs ==int(zon_cs * 0.3)):
            subDir2 = "tt/"
        elif ( zhen_cs  <int(zon_cs * 0.2) or (zhen_cs<int(zon_cs * 0.8)and zhen_cs>int(zon_cs * 0.4))):
            subDir2 = "tr/"
        ...
        if (subDir2 == "tt/"):
            ...
            #     print(c12,"zero",)
            #     pdb.set_trace()
            writeData(jmu[int(zon_cs * 0.2) * slice_size:int(zon_cs * 0.2) * slice_size + 1200000], file_form,
                      filePath_wav_mix_sum, fbl + str(zhen_cs) + "")
            writeData(c12[int(zon_cs * 0.2) * slice_size:int(zon_cs * 0.2) * slice_size + 1200000], file_form,
                      filePath_wav_signal_sum, fbl + str(zhen_cs) + "")
            writeData(blue_noise[int(zon_cs * 0.2) * slice_size:int(zon_cs * 0.2) * slice_size + 1200000], file_form,
                      filePath_wav_noise_sum, fbl + str(zhen_cs) + "")
            writeData(hang[int(zon_cs * 0.2) * slice_size:int(zon_cs * 0.2) * slice_size + 1200000], file_form,
                      filePath_wav_hang_sum, fbl + str(zhen_cs) + "")

            # writeData(jmu[int(zon_cs * 0.2)*slice_size:int(zon_cs * 0.4)*slice_size], file_form, filePath_wav_mix_sum, fbl + str(zhen_cs) + "")
            # writeData(c12[int(zon_cs * 0.2)*slice_size:int(zon_cs * 0.4)*slice_size], file_form, filePath_wav_signal_sum, fbl + str(zhen_cs) + "")
            # writeData(blue_noise[int(zon_cs * 0.2)*slice_size:int(zon_cs * 0.4)*slice_size], file_form, filePath_wav_noise_sum, fbl + str(zhen_cs) + "")
            # writeData(hang[int(zon_cs * 0.2)*slice_size:int(zon_cs * 0.4)*slice_size], file_form, filePath_wav_hang_sum, fbl + str(zhen_cs) + "")
...
def pre_out():
    ...
    # hang_poisition = gethangInfornamtion(pre_hang_original, bin_standard)
    ...
    fuxian(zscc_blue, fbl + "fuxian_blue0",10)
    fuxian_taiyuan(zscc_blue, fbl + "fuxian_blue1",100)
...
# aac=read4CSV_taiyuan_vga2_mix("/home/chenyang/wave-tasnet/voice_data/dataset_taiyuan/文字tek0000ALL.csv")
# np.savetxt("./wenzhi_tek0000ALL.txt",aac[0:5000000])
#
# subset_folder="./voice_data/dataset_taiyuan/ppt太原"
# for _, folders, files in os.walk(subset_folder):
#     for file_name in sorted(files):
#         if file_name.endswith('.csv'):
#             fbl = file_name
#             print("fbl", fbl)
#             subset111_folder = op.join(subset_folder, file_name)
#             line_frame, blue_frame, jmu_frame = read4CSV_taiyuan_vga2_usb(subset111_folder)
#             zon_cd = len(jmu_frame['frame'])
#             getDataset_full_30()

aac=read4CSV_taiyuan_vga2_mix("./voice_data/dataset_taiyuan/ppt太原/tek0002单通道混合主机总线pptALL.csv")
print("长度",len(aac))
test_cd=600000
for ai in range(1,2):
    writeData((aac[0:(ai + 1) * test_cd]), "txt", "./voice_data/wav/tt/mix/",
              "Write data mixtestdata" + str(ai))
    writeData((aac[0:(ai + 1) * test_cd]), "txt", "./voice_data/wav/tt/s1/",
              "Write data mixtestdata" + str(ai))
    writeData((aac[0:(ai + 1) * test_cd]), "txt", "./voice_data/wav/tt/s2/",
              "Write data mixtestdata" + str(ai))
    writeData((aac[0:(ai + 1) * test_cd]), "txt", "./voice_data/wav/tt/s3/",
              "Write data mixtestdata" + str(ai))
    np.savetxt("./voice_data/wav/tt/mix/主机总线mixtest"+str(ai)+".txt",aac[0:(ai+1)*test_cd])
    np.savetxt("./voice_data/wav/tt/s1/主机总线mixtest"+str(ai)+".txt",aac[0:(ai+1)*test_cd])
    np.savetxt("./voice_data/wav/tt/s2/主机总线mixtest"+str(ai)+".txt",aac[0:(ai+1)*test_cd])
    np.savetxt("./voice_data/wav/tt/s3/主机总线mixtest"+str(ai)+".txt",aac[0:(ai+1)*test_cd])

    # np.savetxt("./voice_data/wav/tt/mix/主机总线mixtest"+str(ai)+".txt",aac[ai*len(aac)//4:(ai+1)*len(aac)//4])
    # np.savetxt("./voice_data/wav/tt/s1/主机总线mixtest"+str(ai)+".txt",aac[ai*len(aac)//4:(ai+1)*len(aac)//4])
    # np.savetxt("./voice_data/wav/tt/s2/主机总线mixtest"+str(ai)+".txt",aac[ai*len(aac)//4:(ai+1)*len(aac)//4])
    # np.savetxt("./voice_data/wav/tt/s3/主机总线mixtest"+str(ai)+".txt",aac[ai*len(aac)//4:(ai+1)*len(aac)//4])

# aac=read4CSV_taiyuan_vga2_mix("/home/chenyang/wave-tasnet-taiyuan/voice_data/dataset_taiyuan/ppt太原/单通道混合/tek0003单通道混合屏幕总线pptALL.csv")
# print("长度",len(aac))
#5000000
# for ai in range(4):
#     np.savetxt("./voice_data/wav/tt/mix/屏幕总线mixtest5000000"+str(ai)+".txt",aac[ai*len(aac)//4:(ai+1)*len(aac)//4])
#     np.savetxt("./voice_data/wav/tt/s1/屏幕总线mixtest5000000"+str(ai)+".txt",aac[ai*len(aac)//4:(ai+1)*len(aac)//4])
#     np.savetxt("./voice_data/wav/tt/s2/屏幕总线mixtest5000000"+str(ai)+".txt",aac[ai*len(aac)//4:(ai+1)*len(aac)//4])
#     np.savetxt("./voice_data/wav/tt/s3/屏幕总线mixtest5000000"+str(ai)+".txt",aac[ai*len(aac)//4:(ai+1)*len(aac)//4])


# line_frame, blue_frame, jmu_frame = read4CSV_5_11_18_24("/home/chenyang/wave-tasnet/voice_data/11.csv")
# zon_cd=len(jmu_frame['frame'])
# # hang_bin_standard="pre_out_taiyuan" ###决定二值化标准
# getDataset_full_30()

#
# zmb2=readLvm("/home/chenyang/wave-tasnet/voice_data/jmdx (1).lvm")
# np.savetxt("./cmm_jmdx1mix.txt",zmb2['frame'])
...
# wjj_temp="tmp"
# pre_name="主机总线mixtest50000002"
# # pre_name="电源线tek0000ALL.csv7"
#
...
# # huatu(pre_blue,"pre_blue")
#
# pre_blue_original_s1_path="./voice_data/wav/tt/s1/"+pre_name+  ".txt"
# pre_blue_original=np.loadtxt(pre_blue_original_s1_path)
# # huatu(pre_blue_original,"pre_blue_original")
#
# pre_hang_original_s3_path= "./voice_data/wav/tt/s3/"+pre_name+  ".txt"
# pre_hang_original=np.loadtxt(pre_hang_original_s3_path)
# # huatu(pre_hang_original,"pre_hang_original")
#
...
#     # print("信噪比SNR",snr)
#     # print("分离MSE")
#     ps=Mse_value(pre_blue,pre_blue_original,"分离MSE")
#     # print("混合MSE")
#     pn = Mse_value(mix, c12,"混合MSE")
#     snr = -10 * math.log((ps/pn), 10)
#     print("信噪比SNR", snr)
#
# SNR_singlech(pre_mix,pre_blue_original,pre_blue)