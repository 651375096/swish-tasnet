...
def make_enc_dec(
    fb_name,##fb_name（str，className）：用于制作编码器的Filterbank系列和解码器。要在[``'free'``、``'alytic_free'`、，``'param_sinc'``，``'tft'``]。
    n_filters,
    kernel_size,
    stride=None,
    sample_rate=8000.0,
    ...
    **kwargs,
    ...
    # print("fb_name==",fb_name)##fb_name== free
    # print("who_is_pinv==",who_is_pinv)##who_is_pinv== None
    ...
    if who_is_pinv in ["dec", "decoder"]:
        ...
        fb = fb_class(n_filters, kernel_size, stride=stride, sample_rate=sample_rate, **kwargs)#fb== FreeFB()
        enc = Encoder(fb, padding=padding)#enc== Encoder((filterbank): FreeFB() ))
