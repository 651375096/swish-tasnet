...
def forward_hook(module, input, output):
    print("进入forward_hook")
    # print("module",module)
    # print("input",input)
    # print("output",output)

    # huatu(input[0][0], str(module)+"input")
    # huatu(output[0][0], str(module)+"output")


def huatu_tensor(tensor_in,name):
    tensor = tensor_in.cpu()
    tensor = np.array(tensor)
    # tensor=tensor_in.detach().numpy()

    huatu(tensor[0][0],name)
...
class Conv1DBlock(nn.Module):
    ...
    def __init__(
        ...
        **fb_kwargs,
        ...
        self.skip_add = fb_kwargs['skip_add']
        self.convblock = fb_kwargs['convblock']
        ...
        # depth_conv1d_in = nn.Conv1d(
        #     in_chan, hid_chan, kernel_size, padding=padding, dilation=dilation, groups=hid_chan
        # )
        if causal:#causal=False,
            depth_conv1d = nn.Sequential(depth_conv1d, _Chop1d(padding))
    #################
        if self.convblock==0:############原版7.168259241804914
            self.shared_block = nn.Sequential(
                ...
                nn.PReLU(),
                conv_norm(hid_chan),)
        elif self.convblock==1:##############削减模块7.0671999330495625,
            self.shared_block = nn.Sequential(
                in_conv1d,
                depth_conv1d,
                nn.PReLU(),
        ...
        elif self.convblock==2:##############增加深度提前7.123077439838947,
            self.shared_block = nn.Sequential(
                in_conv1d,
                depth_conv1d,
                Swish(),
        ...
        elif self.convblock==3:##############增加深度提前7.219929171370079,
            self.shared_block = nn.Sequential(
                in_conv1d,
                Swish(),
                depth_conv1d,
        ...
        elif self.convblock==4:##############增加深度提前7.0235926114795575,
            self.shared_block = nn.Sequential(
                in_conv1d,
                conv_norm(hid_chan),
                depth_conv1d,
        ...
        elif self.convblock==5:##############增加深度提前超出内存
            self.shared_block = nn.Sequential(
                in_conv1d,
                Swish(),
                conv_norm(hid_chan),
    ...
    ###############
        # self.shared_block [0].register_forward_hook(forward_hook)
        # self.shared_block [1].register_forward_hook(forward_hook)
        # self.shared_block [2].register_forward_hook(forward_hook)
...
class TDConvNet(nn.Module):
    ...
    def __init__(
        ...
        **fb_kwargs,
        ...
        self.bn_chan = bn_chan#128
        self.hid_chan = hid_chan#        hid_chan=512,
        self.skip_chan = skip_chan#128
        ...
        # print("self.skip_add",self.skip_add)

        layer_norm = norms.get(norm_type)(in_chan)##GlobLN()
        bottleneck_conv = nn.Conv1d(in_chan, bn_chan, 1)##bottleneck_conv Conv1d(512, 128, kernel_size=(1,), stride=(1,))
        ...
        self.TCN = nn.ModuleList()###ModuleList()
        self.count=[]


        for r in range(n_repeats):#n_repeats==3
            for x in range(n_blocks):#n_blocks==8
#########
                self.count.append(x)
###############
                if not causal:##causal=False
        ...
        mask_conv_inp = skip_chan if skip_chan else bn_chan  ##skip_chan=128,bn_chan=128,
        mask_conv = nn.Conv1d(mask_conv_inp, n_src * out_chan, 1)##Conv1d(128, 1024, kernel_size=(1,), stride=(1,))
        self.mask_net = nn.Sequential(nn.PReLU(), mask_conv)
        # Get activation function.
        mask_nl_class = activations.get(mask_act)  ###mask_act="swich",    relu
    ...
    def forward(self, mixture_w):
        ...
        for layer in self.TCN:
            ...
            if self.skip_chan:
                ...
                aaa = self.count[count1] + 1
                # print("config['masknet']['skip_add']",config['masknet']['skip_add'])
                # pdb.set_trace()
                if  self.skip_add ==0:
                    skip_connection = skip_connection + skip
                elif self.skip_add==1:
                    for bbb in range(aaa):
                        skip_connection = skip_connection + skip
                elif self.skip_add==2:
                    skip_connection = skip_connection + skip*aaa

                elif self.skip_add==3:
                    skip_connection = skip_connection + skip + skip*aaa
