...
parser.add_argument("--test_dir", type=str,default=r"./voice_data/json/tt" , help="Test directory including the csv files")
...
parser.add_argument("--exp_dir", default=r"exp/tmp/", help="Experiment root")
parser.add_argument("--out_dir", type=str, default=r"out/", help="Directory in exp_dir where the eval results will be stored")
...
#     # s1_name = result["name"]
#     aaa = os.path.splitext(os.path.basename(result["name"]))[0]
#     print("aaa",aaa)
#     return result
...
def main(conf):
    ...
    if not os.path.exists(os.path.join(eval_save_dir, "final_metrics.json")):
        ...
        signal_save_dir = os.path.join(eval_save_dir, r"signal/")
        original_save_dir = os.path.join(signal_save_dir, r"original/")
        original_mix_save_dir = os.path.join(original_save_dir, r"mix/")
        original_s1_save_dir = os.path.join(original_save_dir, r"s1/")
        original_s2_save_dir = os.path.join(original_save_dir, r"s2/")
        original_s3_save_dir = os.path.join(original_save_dir, r"s3/")

        estimate_save_dir = os.path.join(signal_save_dir, r"estimate/")
        estimate_s1_save_dir = os.path.join(estimate_save_dir, r"s1/")
        estimate_s2_save_dir = os.path.join(estimate_save_dir, r"s2/")
        estimate_s3_save_dir = os.path.join(estimate_save_dir, r"s3/")
...
###########    # print('Is a all zeros?: ', np.all(est_sources_np[2] == 0))
            ...
            if idx in save_idx:
                ...
                mix_name = os.path.splitext(os.path.basename(mix_name))[0]
...
############
                ...
                for src_idx, src in enumerate(sources_np):
                    ...
                    if src_idx == 0:
                        ...
                        s1_name = os.path.splitext(os.path.basename(s1_name))[0]
                        ...
                        s2_name = os.path.splitext(os.path.basename(s2_name))[0]

                        writeData(src,"txt", original_s2_save_dir, s2_name)
                    elif src_idx == 2:
                        #sf.write(original_s2_save_dir + str(idx)+".wav", src, conf["sample_rate"])
                        s3_name = getNameFromPath(test_set.sources[2][idx][0])["name"]
                        s3_name = os.path.splitext(os.path.basename(s3_name))[0]
