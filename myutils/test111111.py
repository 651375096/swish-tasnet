...
max_len = findMaxLength(jmu_info[end_fn]['end'] -jmu_info[start_fn]['start'],
                        # black_info[end_fn]['end']-black_info[start_fn]['start'],
                        blue_info[end_fn]['end']-blue_info[start_fn]['start']
                        )

#获取第一帧的信号
jmu = jmu_frame['signal'][jmu_info[start_fn]['start']:jmu_info[start_fn]['start']+max_len]
# black = black_frame['signal'][black_info[start_fn]['start']:black_info[start_fn]['start']+max_len]
blue = blue_frame['signal'][blue_info[start_fn]['start']:blue_info[start_fn]['start']+max_len]
#line = line_frame['signal'][line_info[fn]['start']:line_info[fn]['start']+max_len]


#将获取的数据图形进行绘制   图片的名字为：变量名+帧号
drawPicture(var_name(jmu)+str(max_len),jmu)
# drawPicture(var_name(black)+str(max_len),black)
drawPicture(var_name(blue)+str(max_len),blue)
...
#将获取的数据图形进行绘制   图片的名字为：变量名+帧号
drawPicture(var_name(mask_blue)+str(max_len),mask_blue)
...
drawPicture(var_name(blue_pure)+str(max_len),blue_pure)
#将数据以lvm格式写入
...
blue_noise = blue_noise_blank
drawPicture(var_name(blue_noise)+str(max_len),blue_noise)
#将数据以lvm格式写入
# write_dir = "./data/reappear/"
# ways = "lvm"
