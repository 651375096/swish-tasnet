...
def fuxian(data,name,beishu=1):
    ...
    # data[np.where(data<np.average(data)/2)]=0
...
def handleBinarization(data,input_standard,deviation=0,way=0):
    ...
    if isNumber(input_standard):
        ...
        # standard =-0.01# (np.max(bin_data)/6)
        # standard=0
        standard = 0.001
...
def get_hang_lbo(data_lbo):
    ...
    :return:信号的帧信息  [{'start': 532368, 'end': 4667857, 'len': 4135489}，...]
    '''
    # huatu(data_lbo[1180:1280],"data_lbo")
    # huatu(data_lbo[1180:10080],"data_lbo")

    # diff_temp_position = handleDifference(data_lbo,0)
    # huatu(diff_temp_position[1180:1280],"差分diff_temp_position")
    # data_abs = np.abs(diff_temp_position)
    data_abs = np.abs(data_lbo)

    huatu(data_abs[1180:1280], "绝对值data_abs")

    bin_stand=(np.max(data_abs)+np.min(data_abs))/10
    print("bin_stand",bin_stand)
    # data_abs_bin_data = handleBinarization(data_abs, input_standard="range")
    # data_abs[np.where(data_abs < 0.012)] = 0  # 二值化 大于标准为0，小于标准为1
    # data_abs[np.where(data_abs < bin_stand)] = 0  # 二值化 大于标准为0，小于标准为1
    # data_abs[np.where(data_abs != 0)] = 1
    huatu(data_abs[1180:1280],"data_abs_bin_data")
    huatu(data_abs[1180:10080],"data_abs_bin_data")
...
def find_long_nonzero_ranges(arr):
    ...
    for i in range(len(arr)):
        if arr[i] != 0:
            ...
            if start is not None and end - start + 1 > 5:
...
def find_0_other_position(data):
    data=np.abs(data)
    temp_position = np.where(data> 0)
    # diff_temp_position = handleDifference(temp_position[0], 0)
    # standard = np.mean(diff_temp_position)
    # position = findhangPartPosition_preout(temp_position[0], standard)
    count0, count1 = 0, 0
    aaaa=find_long_nonzero_ranges(data)
    print(aaaa)


    pdb.set_trace()
    list_position, start, weizhi,start_end = [], [], [],[]
    for i in range(len(data)):
        if data[i] > 0:
            start.append(i)
        #     count1 += 1
        # else:
        #     print(start,count0,count1)
        #     weizhi.append((start[count0] + start[len(start)-1]) // 2)
        #     count0 = count1 + 1
    arr =start
...
def pre_out():
    ...
    fuxian(zscc_blue, fbl + "fuxian_blue",10)
...
# database_path ="/root/autodl-tmp/Dataset_All/VGA_signals/308VGA4_inside"
...
##############
pre_name="1.1-48-4D-7E-BE-C8-651024-768.csv0"
# pre_name="杭电 电磁信息安全_1024_768.lvm"
...
# huatu(pre_hang[600:700],"pre_hang")
# huatu(pre_hang[1180:1280],"pre_hang")

pre_hang_original_s3_path="./voice_data/wav/tt/s3/"+pre_name+".txt"
pre_hang_original=np.loadtxt(pre_hang_original_s3_path)
# huatu(pre_hang_original[600:700],"pre_hang_ooriginal")
# huatu(pre_hang_original[1100:1200],"pre_hang_ooriginal")
# huatu(pre_hang_original[600:70000],"pre_hang_ooriginal")
