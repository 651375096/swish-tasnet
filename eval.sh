...
#python eval_general.py --use_gpu 1 --model DPTNet \
  #--corpus wsj0-mix --test_dir wsj0-mix/2speakers/wav8k/min/tt --task sep_clean \
  #--exp_dir exp/train_dptnet_DPTNet_wsj0-2mix_sep_clean(conf.yml地址) --ckpt_path _ckpt_epoch_196.ckpt --out_dir eval_196_tt;
#exit

echo "exp/train_dptnet_DPTNet_wsj0-2mix_sep_clean_8gpu_pretrained/_ckpt_epoch_199.ckpt";
python eval_general.py --use_gpu 0 --model DPRNNTasNet \
  --corpus wsj0-mix --test_dir /home/yanghailong/speech/data/New/out/tt --task sep_clean \
  --exp_dir exp/tmp  --ckpt_path /home/yanghailong/speech/code/SSL-pretraining-separation-main/exp/tmp/checkpoints/epoch=38-step=742169.ckpt --out_dir out
