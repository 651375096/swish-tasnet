...
def readCSV_mix_s1(path1,path2):
    aaa,bbb=[]*1,[]*1
    with open(path1, 'r', encoding='GBK') as fp:
        reader = csv.reader(fp)
        i = 0
...
def pre_out():
    ...
    fuxian(zscc11, fbl + "pre_blue_original",1)
...
##############
name="wjj1_tx"
# mix_path="/home/chenyang/wave-tasnet/voice_data/dataset_taiyuan/0914/UDP1.csv"
# s1_path="/home/chenyang/wave-tasnet/voice_data/dataset_taiyuan/0914/UDP1_4.csv"
# mix1,s11=readCSV_mix_s1(mix_path,s1_path)
mix1=readCSV_mix("/home/chenyang/wave-tasnet/voice_data/dataset_taiyuan/太原摄像头VGA/新建文件1_4.csv")
# print(mix1)
# huatu(mix1,"mix")
s1=readCSV_mix("/home/chenyang/wave-tasnet/voice_data/dataset_taiyuan/太原摄像头VGA/新建文件1_2.csv")
aaa_cs=6
for aaa in range(aaa_cs):
    print(aaa_cs)
    # np.savetxt("./voice_data/wav/tt/mix/"+name+str(aaa)+".txt",mix1[aaa*len(mix1)//aaa_cs:(aaa+1)*len(mix1)//aaa_cs])
    np.savetxt("./voice_data/wav/tt/s1/"+name+str(aaa)+".txt",s1[aaa*len(mix1)//aaa_cs:(aaa+1)*len(mix1)//aaa_cs])
    # np.savetxt("./voice_data/wav/tt/s2/"+name+str(aaa)+".txt",mix1[aaa*len(mix1)//aaa_cs:(aaa+1)*len(mix1)//aaa_cs])
    # np.savetxt("./voice_data/wav/tt/s3/"+name+str(aaa)+".txt",mix1[aaa*len(mix1)//aaa_cs:(aaa+1)*len(mix1)//aaa_cs])
